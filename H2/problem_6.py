...
while j >= 0:
if itemToInsert < lyst[j]: lyst[j + 1] = lyst[j]
j -= 1
else: break
lyst[j + 1] = itemToInsert i += 1

def quicksort(lyst): 
  quicksortHelper(lyst, 0, len(lyst) - 1)
  
def quicksortHelper(lyst, left, right): """Runs insertionSort if length of list segment is <= 50."""
if right - left <= 50:
insertionSort(lyst, left, right) else:
...
# but the following code shows what is really going on temp = lyst[i]
lyst[i] = lyst[j]
lyst[j] = temp
